...
void _type_init_stack(const void* cpv_input, void* pv_output)
{
    ...
    b_result = _create_stack_auxiliary((cstl_stack_t*)cpv_input, (char*)pv_output);
    assert(b_result);
    stack_init((cstl_stack_t*)cpv_input);
}

void _type_copy_stack(const void* cpv_first, const void* cpv_second, void* pv_output)
{
    assert(cpv_first != NULL && cpv_second != NULL && pv_output != NULL);
    stack_assign((cstl_stack_t*)cpv_first, (cstl_stack_t*)cpv_second);
    *(bool_t*)pv_output = true;
}
...
{
    assert(cpv_input != NULL && pv_output != NULL);
    _stack_destroy_auxiliary((cstl_stack_t*)cpv_input);
    *(bool_t*)pv_output = true;
}
